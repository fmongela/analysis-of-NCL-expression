...
my_packs <- c("readr","ggplot2","tidyverse","rstatix","ggprism",
              "ggthemes","scales","grid","gridExtra","RColorBrewer")
...
#get antibody name
antibody <<- loaded_table %>% slice(11) %>% pull("Antibody") %>% as.character() %>% substring(6)
#rename columns
...
grid.arrange(
  ...
  ncol = 1,
  top = textGrob("Expé #3247",gp=gpar(fontsize=15,font=3))
)
